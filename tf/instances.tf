...
resource "google_compute_instance" "ansible-runner" {
  ...
  provisioner "remote-exec" {
    ...
    "git clone https://github.com/rulet02/ansible_hw_2.git"]
  }
  metadata = {
...
resource "google_compute_instance" "ansible-workers" {
  ...
  metadata_startup_script = "sudo apt-get update && sudo apt-get install -y python python-pip python-setuptools python3 python3-pip python3-setuptools"
  depends_on = [
